...
class UploadImage extends Component {
  ...
  render () {
    ...
    if (
      (team.profile !== null &&
        team.profile.avatar !== undefined &&
        team.profile.avatar !== null &&
        team.profile.avatar !== 'a') ||
      this.state.avatarImage !== null
    ) {
      avatar =
        this.state.avatarImage !== null
          ? this.state.avatarImage
          : `${team.profile.avatar}?${Math.random()}`
    } else {
      avatar =
    ...
    if (
      (team.profile !== null &&
        team.profile.banner !== undefined &&
        team.profile.banner !== null &&
        team.profile.banner !== '') ||
      ...
      banner =
        this.state.bannerImage !== null
          ...
          : `${team.profile.banner}?${Math.random()}`
    } else {
      banner =
    ...
    let handleImage
    this.state.edit === 'avatar'
      ? (handleImage = handleAvatar)
      : (handleImage = handleBanner)
                  ...
                  <div>
                    {(this.state.change ||
                      ...
                      <React.Fragment>
                        ...
                        >
                          <p>
                            Drag-and-drop team {this.state.edit} here or click
                            to select files. <br /> Only *.jpeg and *.png images
                            </p>
                        </Dropzone>
                          <Button
                    ...
                    {!this.state.change && (
                      <React.Fragment>
                        <img
                          src={banner}
                          className={classes.cropBannerPreview}
                    ...
                    {!this.state.change && (
                      <React.Fragment>
                        <img
                          src={avatar}
                          className={classes.cropAvatarPreview}
                          alt='AvatarCrop Preview'
                        />
                        <Button
                    ...
                    {(this.state.avatarImage !== null ||
                      ...
                      <Typography variant='title' color='primary'>
                        Click Update Team to save image changes!
                      </Typography>
                    )}
                  </div>
                  <Dialog
                    ...
                    <div classes={classes.dialogContainer}>
                      <DialogContent>
                        <div
                          className={
                            this.state.edit === 'avatar'
                              ? classes.cropAvatarBox
                              : classes.cropBannerBox
                          }
                        >
                          {this.state.src !== null && (
                            <Cropper
  ...
  async handleCrop (e, getAzureSAS, teamSlug, handleImage) {
    ...
    let imageConfig =
      this.state.edit === 'avatar' ? imageAvatarConfig : imageBannerConfig
    let resizedBlob = await readAndCompressImage(croppedBlob, imageConfig)

    return this.setState(
      { cropped: resizedBlob, cropping: true, open: false },
      () => {
